# Bandgap-core routed layout record: 20260804-091957-25d02e6
...
Mismatch categories: `{"device.unmatched": 45, "net.unmatched": 63}`.
...
- Record ID: `20260804-091957-25d02e6`
...
## Issue #72 bisect note (step 1 of 3)

This record is bisect **step 1** for issue #72 (`main`'s 0/0
...
- `SUBSTRATE_SAME_NETS` set to `[]` (the `[["vsubs", "VSS"]]` `hints.same_nets`
  declaration dropped). Everything else -- `RES_FLAVOR`/`RES_CLASS`
  (`res_high_po`), the bus/route topology -- is unchanged from `main`.

Result: `lvs.combined.json` still reports `devices.matched: 0`,
`nets.matched: 0` (`mismatch_count: 108`, vs. `main`'s own `106` -- see the
baseline record immediately before this one in this directory,
`20260804-090455-25d02e6`, generated from the unmodified `main` state for
this comparison). Dropping the substrate hint does **not** recover
correspondence, and the mismatch count moves slightly the wrong way -- this
rules out `SUBSTRATE_SAME_NETS`/`hints.same_nets` as the cause of the 0/0
regression.
