# Bandgap-core routed layout record: 20260804-043643-f421e4c
...
A `klt gen diff_pair` reports its two transistors as two port families (`M1_*`/`M2_*`, or `Q1_*`/`Q2_*` when `mirror` is false). Which family is which schematic device is *this flow's* choice, not the generator's -- the halves are geometrically interchangeable. Record `20260804-012421-1a97bf5` shows what happens when the choice is never made: every net picked whichever candidate pad sat nearest its own centroid, independently, so `PN` took a finger of the same amp_pmirr half the `AOUT` label named (MP3's drain and MP4's drain were one physical transistor), and amp_nload's `D1` route and `D1_GATE` label disagreed about which half is MN1. Neither is visible to DRC or to the drawn-short check -- every terminal involved is legal, well-separated metal.


A diff_pair's two port families are bound to named schematic devices once, in MOS_HALVES, and every block's bus spec resolves through it. Without that binding the centroid-nearest pick is free to hand two different schematic nodes two fingers of the same physical transistor, which is a topology error that neither DRC nor the drawn-short check can see -- both terminals are legal, well-separated metal.

The binding is made once, in `MOS_HALVES`; the half-to-device column below is read back out of `bus-summary.json`'s `device_labels`, i.e. out of the binding the bus actually drew with, and the net columns are that block's bus spec. Every source, drain and gate of both halves is bussed from the generator's own reported ports, so there is no per-net pad choice left to get wrong.

| block | port family | schematic device | source net | drain net | gate net |
| --- | --- | --- | --- | --- | --- |
| `core_mirror` | `M1_*` | `MPOUT` | `VDD` | `VOUT` | `GDRV` |
| `core_mirror` | `M2_*` | `MPAMP` | `VDD` | `TAIL` | `GDRV` |
| `amp_input_pair` | `Q1_*` | `MP1` | `TAIL` | `D1` | `VB` |
| `amp_input_pair` | `Q2_*` | `MP2` | `TAIL` | `D2` | `VA` |
| `amp_nload` | `M1_*` | `MN1` | `D1` | `VSS` | `D1` |
| `amp_nload` | `M2_*` | `MN2` | `D2` | `VSS` | `D2` |
| `amp_pmirr` | `M1_*` | `MP3` | `VDD` | `PN` | `PN` |
| `amp_pmirr` | `M2_*` | `MP4` | `VDD` | `GDRV` | `PN` |
| `amp_nmirr` | `M1_*` | `MN3` | `VSS` | `GDRV` | `D1` |
| `amp_nmirr` | `M2_*` | `MN4` | `VSS` | `PN` | `D2` |
...
Per-node connectivity proof: every node's drawn met1 is one connected conductor, except the 4 listed here, which close through li1 or are joined by an inter-block hop that did not route: `{"D2": 2, "VB": 2, "VOUT": 2, "VSS": 2}`.

Label-collision proof: **0** extracted net(s) carry more than one label. This is the pad-side counterpart of the check above and is gated the same way. A `pins[]` entry labels a *port*, i.e. a pad, so a label placed on a pad another node's metal already contacts does not name its own node -- it renames that node, and `klt extract` emits the result as a single net called `A|B` with nothing in `warnings[]` and DRC still clean. The previous increment's composed layout shipped exactly that: `VOUT`'s label sat on `core_mirror.M2_1_D`, which is MPAMP's drain and the pad the drawn `TAIL` net contacts, so its extracted netlist contained a net named `TAIL|VOUT` -- the layout asserting that the reference output and the amp tail are one node. PR #67 fixed that by making the pin selector and the router share one claimed-pad set; this increment goes further and retires the gate-port pin labels entirely, because every schematic node is now named on its own drawn met1. The set is still shared (the trim taps still go through it) and this line is still the proof. Filed upstream as 2AMLogic/klayout-tools#470 (the silence, not the collision, is the tool gap).

## Inter-block nets drawn on met1

| net | terminals | routed | schematic node |
| --- | --- | --- | --- |
| `GDRV` | `core_mirror:GDRV trunk` + `amp_pmirr:GDRV trunk` + `amp_nmirr:GDRV trunk` | yes | the amp's output (MP4/MN3 drains) and the core mirror's gate drive -- one node in the schematic. PR #67 drew the two drain ends only, and named the gate end `GDRV_GATE` because a gate could not be contacted; both ends are one drawn conductor now (the `core_mirror` terminal is its gate lane), so the node needs no second name |
...
| `VSS` | `amp_nload:VSS trunk` + `amp_nmirr:VSS trunk` + `pnp_ptat:VSS trunk` + `pnp_ctat:VSS trunk` | NO | VSS trunk: the amp's NMOS rails and guard rings plus both PNP base ties (the diode-connected PNPs' base and collector both sit on VSS) |
...
The table above counts this flow's own routing declaration. This one counts what issue #62 actually asks for: every node of design/bandgap_core.sch (+ design/error_amp.sch) that joins devices in different blocks, and whether drawn metal joins **all** the blocks the schematic says it reaches. A node scored `partial` is drawn between some of its blocks and named, so it is addressable, but it is not connected. The gaps have two causes, stated per row below rather than collapsed into one sentence: a hop this flow's own met1 router could not place without a drawn short, and -- for `VSS`'s three resistor blocks only -- the resistor-flavour gap (RES_FLAVOR_NOTE), where the drawn `res_generic_po` has no bulk terminal to reach at all. The MOS-gate cause that dominated PR #64/#65/#67 is gone: every gate is contacted here (MOS_GATE_NOTE).
...
| `D2` | `amp_input_pair`, `amp_nload`, `amp_nmirr` | `amp_nload`, `amp_nmirr` | `amp_input_pair` | **partial** |
...
| `VSS` | `amp_nload`, `amp_nmirr`, `pnp_ctat`, `pnp_ptat`, `res_r2`, `res_trim`, `res_r1` | `amp_nmirr`, `pnp_ctat`, `pnp_ptat` | `amp_nload`, `res_r1`, `res_r2`, `res_trim` | **partial** |
...
| LVS | mismatch | mismatch_count=81 |
...
- **Not LVS-clean.** `klt lvs` reports `mismatch` with `mismatch_count=81` against the xschem-derived reference netlist. See the five disclosed causes above; the largest is this flow's own router leaving 4 declared net(s) undrawn. It is no longer the gate-contact gap: PR #67's record could say `devices.matched` was 0 and would stay 0 however much more metal was drawn, because no split group could collapse into the m=N device the schematic states. Every gate is contacted here, so that ceiling is lifted and what remains is measured, not structural.
- **Not fully inter-block routed either.** 8/12 schematic inter-block nets are joined across every block they reach. The rest are *partial*, not absent: each is drawn between the blocks whose terminals are reachable and stops at the one that is not, which the coverage table names per row. `VSS` stops at the resistor blocks' missing bulk terminal (RES_FLAVOR_NOTE); the others stop at a hop this flow's own router could not place without a drawn short.
...
- Record ID: `20260804-043643-f421e4c`
- `klt` version: `klt 0.1.0` (pinned, see `layout/requirements.txt`)
- KLayout engine version: `0.30.10`
- Repo state: `f421e4c08bcec401474c3d40509af92cfcc35c9d` on `feature/issue-62` (dirty)
