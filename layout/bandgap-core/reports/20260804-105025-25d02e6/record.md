# Bandgap-core routed layout record: 20260804-105025-25d02e6
...
| extract | ok | device_count=251, device_counts={"nfet": 16, "pfet": 52, "pnp": 24, "res_high_po": 159}, pin_count=16 |
| LVS | mismatch | mismatch_count=92 |
...
| `nfet` | 16 | 0 |
| `pfet` | 52 | 68 |
| `res_high_po` | 159 | 67 (as `res_generic_po`) |
| promoted pins | 16 | 0 |
...
| --- | --- | --- | --- |
| combined | True | mismatch | 92 |
| uncombined | False | mismatch | 468 |
...
Mismatch categories: `{"device.unmatched": 42, "net.merged": 5, "net.split": 45}`.

The residual gap has six disclosed causes, none of them a topology error in either netlist:
...
6. **No resistor can be paired at all: the two sides' resistor device class has a different terminal count.** The sky130 deck marks `res_high_po` `bulk_to_substrate`, so `klt extract` writes a **three-node** R card (`R<name> <a> <b> <bulk> <value> <model>`), which KLayout's SPICE reader turns into `DeviceClassResistorWithBulk` (terminals A/B/W). `reference.spice` states the schematic, where a poly resistor is a two-node device, so the same reader turns its R cards into `DeviceClassResistor` (terminals A/B). Same model name on both sides, different terminal count -- `NetlistComparer` cannot pair them, and it says so only as generic `device.unmatched` entries, with no `device_class_mismatch` event and nothing in `device_classes[]` distinguishing the two. `klt lvs` offers no request-side hook to reconcile the arity (`hints.same_nets` reconciles a *net*, which is enough for MOS bodies because M cards carry four nodes on both sides). The only workaround available today is to add a bulk node to the reference's R cards, i.e. to stop the reference being a transcription of the schematic -- which this flow refuses to do for the same reason it refuses every other reference edit. Filed upstream as 2AMLogic/klayout-tools#504. This is why cause 5's value difference has never actually been reached -- the comparer stops one step earlier, at the arity.

None of the six is worked around by editing either netlist. `reference.spice` states design/bandgap_core.sch; rewriting it to enumerate the layout's own shortfalls would make LVS compare the layout against itself, which is not evidence. The one declaration made -- the substrate correspondence in cause 2 -- is a `hints` entry, and it states something that is true of the design rather than something convenient about the layout.
...
- **Not LVS-clean.** `klt lvs` reports `mismatch` with `mismatch_count=92` against the xschem-derived reference netlist, and `devices.matched` is 3. The six causes above are the whole of it; none is hidden behind a number that moved.
...
- **Array dummies are counted as real devices.** The `pnp` and `res_high_po` counts above include each array's dummy edge units, which have no schematic counterpart and cannot be marked as dummies (cause 2 above). Turning dummies off would trade a real matching property for a smaller mismatch number; this flow keeps them.

## Provenance

- Record ID: `20260804-105025-25d02e6`
...
## Issue #72 fix note: what the 0/0 correspondence regression actually was

This record is the **fix run** for issue #72. It is the first record in this
directory with a non-zero `klt lvs` correspondence: `devices.matched: 3`,
`nets.matched: 1` (`TAIL`), `mismatch_count: 92` -- against `main`'s
`0`/`0`/`106` -- with the schematic's own `res_high_po` resistor class and
the `hints.same_nets` substrate declaration both **unchanged**, and with
`layout/bandgap-core/reference.spice` untouched.

### The bisect ruled out both suspects

| run | changed variable(s) | `devices.matched` | `nets.matched` | `mismatch_count` |
| --- | --- | --- | --- | --- |
| `20260804-090455-25d02e6` | none (baseline, unmodified `main`) | 0 | 0 | 106 |
| `20260804-091957-25d02e6` | `SUBSTRATE_SAME_NETS = []` | 0 | 0 | 108 |
| `20260804-100636-25d02e6` | `RES_FLAVOR`/`RES_CLASS` -> generic | 0 | 0 | 108 |
| `20260804-101943-25d02e6` | both of the above | 0 | 0 | 110 |
| **this record** | **the `VDD` n-well tap fix below** | **3** | **1** | **92** |

Neither the substrate hint nor the resistor flavour is the cause. Note that
step 2 makes things *worse*, not better: drawing the ladder as
`res_generic_po` while the reference names the schematic's `res_high_po` adds
two `topology` mismatches and recovers nothing -- so there was never a case
for keeping it.

### The cause: two PMOS groups' n-well taps were on floating nets

The `VDD` trunk is declared with five terminals, three of which are the PMOS
groups' guard-ring bulk taps (`bulk_terminal()`). Two of its hops did not
route:

```
VDD routed=False
  FAILED core_mirror.TAP_S   -> core_mirror:VDD:far1   blocked_by=GDRV
  FAILED amp_pmirr.TAP_S     -> amp_input_pair.TAP_S   blocked_by=TAIL
```

So `core_mirror`'s and `amp_input_pair`'s n-wells were never joined to `VDD`,
and `klt extract` put those two groups' PMOS body terminals on **anonymous,
unnamed nets** (`$187`/`$188` in the baseline record's own
`bandgap_core_routed.extract.spice`), while `amp_pmirr`'s -- whose tap did
route -- sat correctly on `VDD`:

```
$17 PFET  S=TAIL G=VA   D=D2   B=$187      <- amp_input_pair, floating n-well
$33 PFET  S=VDD  G=GDRV D=TAIL B=$188      <- core_mirror,    floating n-well
$53 PFET  S=VDD  G=PN   D=PN   B=VDD       <- amp_pmirr,      correct
```

The reference puts every PMOS bulk on `VDD` (`... VDD VDD pfet`).
`NetlistComparer` is a graph matcher with no name-based seeding, so a netlist
whose PMOS bodies are on nets the reference does not contain gives it nothing
to seed from -- and the failure is *global*, not local: zero devices and zero
nets matched anywhere in the circuit, including the transistors and bipolars
that were drawn correctly.

Isolated from the baseline record's own extracted netlist, without re-running
the flow: substituting `VDD` for `$187`/`$188` in
`20260804-090455-25d02e6/bandgap_core_routed.extract.spice` and re-running the
same compare takes it from `0`/`0` to `5`/`1` immediately. Substituting only
one of the two leaves it at `0`/`0` -- both n-wells have to land on `VDD`
before the matcher gets a foothold. The same surgery applied to the resistor
side instead (dropping the third node from every extracted `R` card, i.e.
what `res_generic_po` would have produced) leaves it at `0`/`0`, which is the
analytic form of bisect step 2's result above.

### The fix: the tap is a routing choice, not a table entry

`bulk_terminal()` pinned every block to its `TAP_S` pad. `klt gen diff_pair`
reports three taps per ring (`TAP_N`/`TAP_S`/`TAP_E`), and the two hops above
fail for geometric reasons specific to the south tap: `core_mirror.TAP_S`
sits at the bottom edge of an 8 x 19 um block whose own comb escape stubs
occupy every way out, and `amp_input_pair.TAP_S` is on the far side of a
180 um-wide input pair from the block the trunk arrives from. All three taps
are now offered as *candidates* and the existing candidate/chain search picks
one; it takes `core_mirror.TAP_N` and `amp_input_pair.TAP_N`, and `VDD`
routes end to end. Every PMOS bulk in this record's extracted netlist is on
`VDD`.

Cost, stated rather than buried: the corridor `VDD` now uses is one `GDRV`
previously had, so `GDRV` is short a hop where `VDD` used to be. The
schematic-coverage count is unchanged at **9/12** -- the same number as the
baseline, with a different net in the short column -- and it is a good trade,
because the coverage count is the same and the LVS correspondence is not.
`GDRV`'s remaining hop is the first thing the next increment of #62 should
attack.

### A second, independent blocker found on the way (upstream)

Isolating the above also showed why *no resistor* can ever be paired,
whatever the routing does: the layout's `res_high_po` extracts as a
three-terminal `DeviceClassResistorWithBulk` while the reference's two-node
`R` card reads as a two-terminal `DeviceClassResistor`, and `klt lvs` has no
request-side hook to reconcile the arity and raises no diagnostic naming it.
Filed as **2AMLogic/klayout-tools#504**; stated in full as cause 6 above. It
is not the cause of the 0/0 regression -- the bisect above rules the resistor
class out -- but it is a hard ceiling on criterion 4 of #62 that no amount of
routing will lift.
