# Bandgap-core routed layout record: 20260805-225845-5b37cf5
...
| 2 | Resistor ladder at real unit count | MET | `res_r2` num=92 (= 2 legs x 46 coarse 5um units) + `res_trim` num=40 (= 2 legs x 20 fine 1um units) = 250 um/leg at DR-002 code 0, against design/bandgap_core.sch's `r_lseg*n_r2` = 250 um (delta +0 um); composed bbox 45,968 um^2 vs 50,000 um^2 budget |
| 3 | Extract: correct device classes + promoted pins | MET | device_counts={"nfet": 16, "pfet": 52, "pnp": 16, "res_high_po": 139}, pin_count=11 |
| 4 | `klt lvs` clean | NOT MET | status=mismatch, mismatch_count=1 |
...
- [x] Composed bbox area (45,968 um^2) is within the < 0.05 mm^2 (50,000 um^2) budget, **at the real full-length ladder count** (92 coarse + 40 fine units)
...
| `res_r2` | `res_array` | R2A/R2B interdigitated ladder (K = R2/R1 divider) | 46 coarse 5um segments PER LEG x 2 legs = 92 total = 230 um/leg, the coarse part of design/bandgap_core.sch's 250 um (`r_lseg*n_r2` at n_r2=50); the remaining 20 um is `res_trim`'s fine ladder, so the trim taps subtract from the specified length instead of adding to it (issue #91). The skeleton's 16-unit reduction is closed by `res_array`'s `rows` fold parameter (2AMLogic/klayout-tools#415, merged via #418) |
| `res_trim` | `res_array` | Downward-only trim ladder taps (both legs) | 20 fine 1um units PER LEG x 2 legs = 40 unit taps, the fine part of the same 250 um leg -- code 0 puts all 20 in circuit and code -k skips k of them, so the ladder spans 0..-20 of which DR-002 certifies 0..-16 (design/bandgap_core.sch CORE_PARAMS, DR-002); drawn 1:1 |
...
| `res_r2` | 2 interdigitated series string(s) | 90 unit-to-unit met1 links |
...
| `TRIM_A` | `res_r2.R90_B` + `res_trim.R0_A` | yes | met1 | R2A's coarse 230 um into leg A of the fine trim ladder that carries the leg's last 20 um (DR-002, downward-only) |
...
| `TRIM_B` | `res_r2.R91_B` + `res_trim.R1_A` | yes | met1 | R2B's coarse 230 um into leg B of the same fine ladder |
...
**6** of this cell's inter-block hops are drawn on met2 rather than met1, each entered and left through a via1 stack (met1 pad + `via.drawing` cut + met2 pad). met1 on this floorplan carries both every block's intra-block bus and every inter-block net, and the hops below had no met1 corridor at any lane, margin, block placement or search depth this repo can set -- layout/matching-plan.md Sections 7d-7o are the exhausted list. met2 is a genuinely independent conductor, and became one for sky130's curated deck only with 2AMLogic/klayout-tools#508 (merged via #511); before that its `metal2` role resolved to the same met1 layer this flow's own bussing already occupies. The escape is tried **strictly last**, after every met1 elbow, channel path and Z-detour has been drawn and rolled back, so met1 remains the primary plane -- see MET2_ESCAPE_NOTE.
...
| `-1` | res_trim.R36_B | res_trim.R37_B | 249 um | yes |
| `-2` | res_trim.R34_B | res_trim.R35_B | 248 um | yes |
| `-3` | res_trim.R32_B | res_trim.R33_B | 247 um | yes |
| `-4` | res_trim.R30_B | res_trim.R31_B | 246 um | yes |
| `-5` | res_trim.R28_B | res_trim.R29_B | 245 um | yes |
| `-6` | res_trim.R26_B | res_trim.R27_B | 244 um | yes |
| `-7` | res_trim.R24_B | res_trim.R25_B | 243 um | yes |
| `-8` | res_trim.R22_B | res_trim.R23_B | 242 um | yes |
| `-9` | res_trim.R20_B | res_trim.R21_B | 241 um | yes |
| `-10` | res_trim.R18_B | res_trim.R19_B | 240 um | yes |
| `-11` | res_trim.R16_B | res_trim.R17_B | 239 um | yes |
| `-12` | res_trim.R14_B | res_trim.R15_B | 238 um | yes |
| `-13` | res_trim.R12_B | res_trim.R13_B | 237 um | yes |
| `-14` | res_trim.R10_B | res_trim.R11_B | 236 um | yes |
| `-15` | res_trim.R8_B | res_trim.R9_B | 235 um | yes |
| `-16` | res_trim.R6_B | res_trim.R7_B | 234 um | yes |
| `-17` | res_trim.R4_B | res_trim.R5_B | 233 um | **no -- out of certified range** |
| `-18` | res_trim.R2_B | res_trim.R3_B | 232 um | **no -- out of certified range** |
| `-19` | res_trim.R0_B | res_trim.R1_B | 231 um | **no -- out of certified range** |
| `-20` | res_trim.R0_A | res_trim.R1_A | 230 um | **no -- out of certified range** |
...
| quantity | value |
| --- | --- |
| `res_r2` coarse leg (drawn) | 230 um (46 x 5 um) |
...
| met2 DRC (this repo's own) | clean | violation_count=0, via1 cuts=10, met2 polygons=4 |
| extract | ok | device_count=223, device_counts={"nfet": 16, "pfet": 52, "pnp": 16, "res_high_po": 139}, pin_count=11 |
| LVS | mismatch | mismatch_count=1 |
...
| `nfet` | 16 | 0 |
| `pfet` | 52 | 68 |
| `res_high_po` | 139 | 67 (as `res_generic_po`) |
| promoted pins | 11 | 0 |
...
| --- | --- | --- | --- |
| combined | True | mismatch | 1 |
| uncombined | False | mismatch | 397 |
...
- Record ID: `20260805-225845-5b37cf5`
- `klt` version: `klt 0.2.0` (pinned, see `layout/requirements.txt`)
- KLayout engine version: `0.30.10`
- Repo state: `5b37cf55c825e27947d4d22f5d55a0f81aa63c54` on `feature/issue-108` (dirty)
