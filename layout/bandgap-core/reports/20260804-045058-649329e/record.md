# Bandgap-core routed layout record: 20260804-045058-649329e
...
| 4 | `klt lvs` clean | NOT MET | status=mismatch, mismatch_count=106 |
...
| `VA` | `res_trim.R30_B` + `pnp_ctat:VA trunk` + `amp_input_pair:VA:far0` | yes | the R2A leg's low end (through its trim taps) to Q1's emitter bus and MP2's gate -- the amp's VINN node |
...
| `VDD` | `core_mirror:VDD:far0` + `amp_pmirr:VDD:spine0` + `core_mirror.TAP_S` + `amp_input_pair.TAP_S` + `amp_pmirr.TAP_S` | NO | VDD trunk: MPOUT/MPAMP and MP3/MP4 sources -- every finger of all four, not one pad per block -- plus each PMOS group's n-well guard-ring tap (the reference's pfet bulk terminal) |
| `VSS` | `amp_nload:VSS:far1` + `amp_nmirr:VSS:far0` + `amp_nload.TAP_S` + `amp_nmirr.TAP_S` + `pnp_ctat:VSS trunk` + `pnp_ptat:VSS trunk` | NO | VSS trunk: every finger of all four amp NMOS sources (MN1-MN4), both NMOS groups' substrate guard-ring taps, and both PNP base ties (the diode-connected PNPs' base sits on VSS) |
...
| `D1` | `amp_input_pair:D1:far1` + `amp_nload:D1:far0` + `amp_nmirr:D1:far0` | NO | MP1's drain, MN1's diode-connected drain/gate, and MN3's gate |
...
| LVS | mismatch | mismatch_count=106 |
...
| --- | --- | --- | --- |
| combined | True | mismatch | 106 |
| uncombined | False | mismatch | 469 |
...
Mismatch categories: `{"device.unmatched": 45, "net.merged": 10, "net.split": 51}`.
...
- **Not LVS-clean.** `klt lvs` reports `mismatch` with `mismatch_count=106` against the xschem-derived reference netlist, and `devices.matched` is 0. The five causes above are the whole of it; none is hidden behind a number that moved.
...
- Record ID: `20260804-045058-649329e`
- `klt` version: `klt 0.1.0` (pinned, see `layout/requirements.txt`)
- KLayout engine version: `0.30.10`
- Repo state: `649329e51950df835729f39003bb4f32d6148199` on `feature/issue-62` (dirty)
