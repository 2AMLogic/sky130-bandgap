# Bandgap-core routed layout record: 20260923-070209-dbd57a9
...
| --- | --- | --- | --- |
| 1 | Full inter-block routing | MET | 14/14 **schematic** inter-block nets fully drawn (15/15 declared met1 nets routed, 0 unrouted) -- see "Schematic inter-block nets" below |
| 2 | Resistor ladder at real unit count | MET | `res_r2` num=98 (= 2 legs x 49 coarse 5um units) + `res_trim` num=40 (= 2 legs x 20 fine 0.5um units) = 255 um/leg at DR-002 code 0, against design/bandgap_core.sch's `r_lseg*n_r2` = 255 um (delta +0 um); composed bbox 70,360 um^2 vs 80,000 um^2 budget |
| 3 | Extract: correct device classes + promoted pins | MET | device_counts={"nfet": 19, "pfet": 54, "pnp": 24, "res_high_po": 145}, pin_count=13 |
...
- [x] Composed bbox area (70,360 um^2) is within the < 0.05 mm^2 (80,000 um^2) budget, **at the real full-length ladder count** (98 coarse + 40 fine units)

## Flow

1. `klt gen` once per matched device group (16 blocks).
...
| `su_ref` | `M1_*` | `MPC1` | `M1_*_D` | `M1_*_S` |
| `su_ref` | `M2_*` | `MPC2` | `M2_*_D` | `M2_*_S` |
...
| `res_r2` | `res_array` | R2A/R2B interdigitated ladder (K = R2/R1 divider) | 49 coarse 5um segments PER LEG x 2 legs = 98 total = 245 um/leg, the coarse part of design/bandgap_core.sch's 255 um (`r_lseg*n_r2` at n_r2=51); the remaining 10 um is `res_trim`'s fine ladder, so the trim taps subtract from the specified length instead of adding to it (issue #91). The skeleton's 16-unit reduction is closed by `res_array`'s `rows` fold parameter (2AMLogic/klayout-tools#415, merged via #418) |
| `res_trim` | `res_array` | Downward-only trim ladder taps (both legs) | 20 fine 0.5um units PER LEG x 2 legs = 40 unit taps, the fine part of the same 255 um leg -- code 0 puts all 20 in circuit and code -k skips k of them, so the ladder spans 0..-20 of which DR-002 certifies 0..-16 (design/bandgap_core.sch CORE_PARAMS, DR-002); drawn 1:1 |
...
| `amp_input_pair` | `diff_pair` | MP1/MP2 (amp PMOS input pair) | amp_m_in=8, W=20 L=10 (design/error_amp.sch); drawn 1:1 -- the dominant mismatch contributor per layout/matching-plan.md Section 1 |
...
| `su_clamp` | `mos_array` | MNC (railed-branch clamp NMOS, issue #52) | m_clamp=1, W=4 L=1 (design/startup_injector.sch); drawn 1:1 as a single unit device -- it pairs with nothing else in the cell |
| `su_sense` | `mos_array` | MNS (VBE-referenced sense NMOS) | m_sense=1, W=48 L=0.5 (design/startup_injector.sch); drawn 1:1 as a single unit device, gate on the core's own VOUT (the schematic's VSENSE input) |
| `pnp_su` | `bjt_array` | QS (startup threshold PNP, small unit W0p68L0p68) | m_pnp=8 sky130_fd_pr__pnp_05v5_W0p68L0p68 (design/startup_injector.sch); drawn 1:1 (8 real units, 2x4 common-centroid) -- the identical unit device and count as `pnp_ctat`, so it is drawn with identical params |
| `su_ref` | `diff_pair` | MPC1/MPC2 (two-high diode-connected PMOS reference, fed from GDRV) | m_ref=1, W=1 L=20 each (design/startup_injector.sch); drawn 1:1 -- the injector's only matched pair, and the only one of its six devices a `diff_pair` can express. NC1, the node between the two diodes, is entirely internal to this block and is drawn by its own intra-block comb |
| `su_inj` | `mos_array` | MNI (degenerate-state eviction NMOS) | m_inj=1, W=10 L=0.5 (design/startup_injector.sch); drawn 1:1 as a single unit device |
...
| `res_r2` | 2 interdigitated series string(s) | 96 unit-to-unit met1 links |
...
| `amp_input_pair` | split-device finger bus | `D1` = 8 finger pads joined on the W spine; `D2` = 8 finger pads joined on the W spine; `VB` = 8 finger pads (8 gate contacts) joined on the W spine; `VA` = 8 finger pads (8 gate contacts) joined on the W spine; `TAIL` = 16 finger pads joined on the W spine |
...
| `pnp_su` | parallel unit bus | `NE` = 8 pads on 4 columns; `VSS` = 8 pads on 4 columns |
| `su_ref` | split-device finger bus | `NC1` = 3 finger pads (1 gate contacts) joined on the W spine; `NG` = 2 finger pads (1 gate contacts) joined on the W spine; `GDRV` = 1 finger pads joined on the W spine |
...
| --- | --- | --- | --- | --- |
| `TRIM_B` | `res_r2.R97_B` + `res_trim.R1_A` | yes | met1 | R2B's coarse 240 um into leg B of the same fine ladder |
| `VB` | `res_trim.R39_B` + `amp_input_pair:VB:far0` + `res_r1.R0_A` | yes | met1 | the R2B leg's low end (at trim code 0, the far end of its fine ladder) to R1's head and MP1's gate -- the amp's VINP node |
| `VBQ` | `res_r1.R6_B` + `pnp_ptat:VBQ trunk` | yes | met1 | R1's tail to Q2's emitter bus |
| `VDD` | `su_clamp.U0_D` + `amp_pmirr:VDD:spine1` + `core_mirror.TAP_N` + `core_mirror:VDD:far1` + `su_ref.TAP_S` + `amp_pmirr.TAP_N` + `amp_input_pair.TAP_N` + `amp_cc.TAP_S` + `amp_cc:VDD:far0` | yes | met1 + met2 x2 | VDD trunk: MPOUT/MPAMP and MP3/MP4 sources, MCC's drain+source (it is wired D=S=B=VDD, a MOS capacitor) -- every finger of all five, not one pad per block -- plus each PMOS group's n-well guard-ring tap (the reference's pfet bulk terminal), and the startup injector's MNC drain and MPC1/MPC2 n-well tap |
| `VSS` | `pnp_ctat:VSS trunk` + `pnp_su:VSS trunk` + `su_inj.U0_S` + `amp_nload:VSS:far1` + `amp_nmirr:VSS:far0` + `amp_nload.TAP_N` + `amp_nmirr.TAP_S` + `pnp_ptat:VSS trunk` | yes | met1 + met2 x2 | VSS trunk: every finger of all four amp NMOS sources (MN1-MN4), both NMOS groups' substrate guard-ring taps, both core PNP base ties (the diode-connected PNPs' base sits on VSS), and the startup injector's MNI source and QS base tie |
| `TAIL` | `core_mirror:TAIL:far0` + `amp_input_pair:TAIL:spine0` | yes | met1 | MPAMP drain to the amp input pair's common source |
| `GDRV` | `su_clamp.U0_S` + `core_mirror:GDRV:far1` + `su_ref:GDRV:spine0` + `su_inj.U0_D` + `amp_cc:GDRV:spine0` + `amp_pmirr:GDRV:far1` + `amp_nmirr:GDRV:far1` | yes | met1 + met2 x3 | the amp's output -- MP4's and MN3's drains -- the core mirror's gate drive, MCC's gate (the compensation cap sits from AOUT/GDRV to VDD), and the startup injector's MPC1 source / MNI drain / MNC source: one node in the schematic and one drawn node in the layout |
| `D1` | `amp_input_pair:D1:far1` + `amp_nload:D1:far1` + `amp_nmirr:D1:far0` | yes | met1 + met2 x1 | MP1's drain, MN1's diode-connected drain/gate, and MN3's gate |
| `D2` | `amp_nload:D2:far1` + `amp_input_pair:D2:far1` + `amp_nmirr:D2:far0` | yes | met1 + met2 x1 | MP2's drain, MN2's diode-connected drain/gate, and MN4's gate |
| `PN` | `amp_pmirr:PN:far0` + `amp_nmirr:PN:far0` | yes | met1 + met2 x1 | MN4's drain, MP3's diode-connected drain/gate, and MP4's gate |
| `NG` | `su_sense.U0_D` + `su_ref:NG:far1` + `su_inj.U0_G` | yes | met1 + met2 x2 | MPC2's diode-connected drain/gate, MNS's drain and MNI's gate -- the injector's internal gate-drive node, held high by the PMOS reference while the core is dead and pulled down by MNS once VOUT clears a VBE |
| `NE` | `su_sense.U0_S` + `pnp_su:NE trunk` | yes | met1 | MNS's source on QS's emitter -- the diode-connected PNP that references the release threshold to a VBE instead of to a MOS threshold alone |
...
| `TRIM_A` | `res_r2.R96_B` + `res_trim.R0_A` | yes | met1 | R2A's coarse 240 um into leg A of the fine trim ladder that carries the leg's last 10 um (DR-002, downward-only) |
| `VOUT` | `res_r2.R0_A` + `res_r2.R1_A` + `core_mirror:VOUT:spine1` + `su_clamp.U0_G` + `su_sense.U0_G` | yes | met1 + met2 x1 | MPOUT's drain and the high ends of both divider legs -- the reference output -- plus the startup injector's VSENSE input (MNS's and MNC's gates) |

### The met2 escape plane

**14** of this cell's inter-block hops are drawn on met2 rather than met1, each entered and left through a via1 stack (met1 pad + `via.drawing` cut + met2 pad). met1 on this floorplan carries both every block's intra-block bus and every inter-block net, and the hops below had no met1 corridor at any lane, margin, block placement or search depth this repo can set -- layout/matching-plan.md Sections 7d-7o are the exhausted list. met2 is a genuinely independent conductor, and became one for sky130's curated deck only with 2AMLogic/klayout-tools#508 (merged via #511); before that its `metal2` role resolved to the same met1 layer this flow's own bussing already occupies. The escape is tried **strictly last**, after every met1 elbow, channel path and Z-detour has been drawn and rolled back, so met1 remains the primary plane -- see MET2_ESCAPE_NOTE.

| net | hop | via1 drops (um) | met2 path |
| --- | --- | --- | --- |
| `VDD` | `core_mirror:VDD:far1` -> `su_ref.TAP_S` | (46.33, 59.09) -> (75.61, 171.42) | 3-point |
| `VDD` | `amp_pmirr.TAP_N` -> `amp_input_pair.TAP_N` | (94.85, 117.85) -> (106.96, 78.62) | 3-point |
| `VSS` | `pnp_ctat:VSS trunk` -> `pnp_su:VSS trunk` | (19.61, 0.31) -> (45.04, 142.37) | 3-point |
| `VSS` | `su_inj.U0_S` -> `amp_nload:VSS:far1` | (99.1, 173.34) -> (167.59, 63.49) | 3-point |
| `GDRV` | `su_clamp.U0_S` -> `core_mirror:GDRV:far1` | (-4.0, 144.06) -> (46.33, 60.69) | 3-point |
| `GDRV` | `core_mirror:GDRV:far1` -> `su_ref:GDRV:spine0` | (46.33, 60.69) -> (61.52, 172.33) | 3-point |
| `GDRV` | `amp_pmirr:GDRV:far1` -> `amp_nmirr:GDRV:far1` | (181.88, 112.22) -> (195.48, 112.053) | 3-point |
| `D1` | `amp_nload:D1:far1` -> `amp_nmirr:D1:far0` | (167.59, 59.49) -> (195.48, 105.9) | 3-point |
| `D2` | `amp_input_pair:D2:far1` -> `amp_nmirr:D2:far0` | (153.99, 64.157) -> (196.28, 104.567) | 4-point |
| `PN` | `amp_pmirr:PN:far0` -> `amp_nmirr:PN:far0` | (181.88, 106.9) -> (195.48, 107.233) | 4-point |
| `NG` | `su_sense.U0_D` -> `su_ref:NG:far1` | (23.18, 173.34) -> (88.7, 174.75) | 3-point |
| `NG` | `su_ref:NG:far1` -> `su_inj.U0_G` | (88.7, 174.75) -> (103.77, 178.95) | 3-point |
| `VA` | `res_trim.R38_B` -> `amp_input_pair:VA:far0` | (136.0, 12.85) -> (153.99, 43.337) | 3-point |
| `VOUT` | `res_r2.R1_A` -> `core_mirror:VOUT:spine1` | (34.69, 1.55) -> (33.91, 63.89) | 3-point |
...
| `VOUT` | `core_mirror`, `res_r2`, `su_sense`, `su_clamp` | `core_mirror`, `res_r2`, `su_clamp`, `su_sense` | -- | **drawn** |
| `GDRV` | `core_mirror`, `amp_pmirr`, `amp_nmirr`, `amp_cc`, `su_ref`, `su_inj`, `su_clamp` | `amp_cc`, `amp_nmirr`, `amp_pmirr`, `core_mirror`, `su_clamp`, `su_inj`, `su_ref` | -- | **drawn** |
| `NG` | `su_ref`, `su_sense`, `su_inj` | `su_inj`, `su_ref`, `su_sense` | -- | **drawn** |
| `NE` | `su_sense`, `pnp_su` | `pnp_su`, `su_sense` | -- | **drawn** |
...
| --- | --- | --- | --- | --- |
| `0` | res_trim.R38_B | res_trim.R39_B | 255 um | yes |
| `-1` | res_trim.R36_B | res_trim.R37_B | 254 um | yes |
| `-2` | res_trim.R34_B | res_trim.R35_B | 254 um | yes |
| `-3` | res_trim.R32_B | res_trim.R33_B | 254 um | yes |
| `-4` | res_trim.R30_B | res_trim.R31_B | 253 um | yes |
| `-5` | res_trim.R28_B | res_trim.R29_B | 252 um | yes |
| `-6` | res_trim.R26_B | res_trim.R27_B | 252 um | yes |
| `-7` | res_trim.R24_B | res_trim.R25_B | 252 um | yes |
| `-8` | res_trim.R22_B | res_trim.R23_B | 251 um | yes |
| `-9` | res_trim.R20_B | res_trim.R21_B | 250 um | yes |
| `-10` | res_trim.R18_B | res_trim.R19_B | 250 um | yes |
| `-11` | res_trim.R16_B | res_trim.R17_B | 250 um | yes |
| `-12` | res_trim.R14_B | res_trim.R15_B | 249 um | yes |
| `-13` | res_trim.R12_B | res_trim.R13_B | 248 um | yes |
| `-14` | res_trim.R10_B | res_trim.R11_B | 248 um | yes |
| `-15` | res_trim.R8_B | res_trim.R9_B | 248 um | yes |
| `-16` | res_trim.R6_B | res_trim.R7_B | 247 um | yes |
| `-17` | res_trim.R4_B | res_trim.R5_B | 246 um | **no -- out of certified range** |
| `-18` | res_trim.R2_B | res_trim.R3_B | 246 um | **no -- out of certified range** |
| `-19` | res_trim.R0_B | res_trim.R1_B | 246 um | **no -- out of certified range** |
| `-20` | res_trim.R0_A | res_trim.R1_A | 245 um | **no -- out of certified range** |
...
| quantity | value |
| --- | --- |
| `res_r2` coarse leg (drawn) | 245 um (49 x 5 um) |
| `res_trim` fine leg at code 0 (drawn) | 10 um (20 x 0.5 um) |
| **total drawn** | **255 um** |
| schematic `L = r_lseg*n_r2 + r_lseg_trim*n_r2_trim` | 255 um |
| delta | +0 um |
...
| Stage | Status | Detail |
| --- | --- | --- |
| met1 routing | routed | nets=15, unrouted=0, drawn-short conflicts=0, split routed nodes=0 |
| DRC | clean | violation_count=0 |
| met2 DRC (this repo's own) | clean | violation_count=0, via1 cuts=26, met2 polygons=12 |
| extract | ok | device_count=242, device_counts={"nfet": 19, "pfet": 54, "pnp": 24, "res_high_po": 145}, pin_count=13 |
...
| `nfet` | 19 | 0 |
| `pfet` | 54 | 68 |
| `res_high_po` | 145 | 67 (as `res_generic_po`) |
| promoted pins | 13 | 0 |
...
- **The R2 leg is drawn one coarse unit longer (issue #178).** DR-003 (RES_HEAD_RESISTANCE_NOTE) established that this flow's drawn decomposition pays the model card's per-instance head/end resistance once per separately-contacted primitive, and issue #99 resized n_r2 54 -> 50 so the DRAWN part's K = R2/R1 landed in spec. What it did NOT do was model that chain in design/bandgap_core.sch, whose XR1/XR2A/XR2B lines stayed single-device -- so the schematic-level harness simulated a K about 9% below the drawn part's and read VOUT(27 degC) ~1.165 V, ~36 mV low. Issue #178 closes that: the schematic now carries an EXACT lumped equivalent of the N-instance chain (a body device plus a replica whose drop is multiplied by N-1, verified identical to an explicit 143-instance chain to 7 significant figures over the whole -40..125 degC sweep), and with one model describing both representations n_r2 was re-derived against DR-005's RATIFIED +/-2 % untrimmed accuracy row (1.176-1.224 V) rather than the superseded draft +/-1 % window: n_r2=50 leaves vref_min ~1.3 mV under the floor at every corner, n_r2=52 puts vref_max over the ceiling at 6 of 15 (process, supply) points, and n_r2=51 sits inside with 10.0 mV bottom / 6.2 mV top margin at all 15 while cutting box TC from 175-182 to 142-159 ppm/degC. n_r1 stays at 7 for RES_RESIZE_NOTE's reason (branch current, hot-corner headroom); no operating-point collapse is seen at ff/2.97 V or fs/2.97 V at either n_r2=51 or 52. Drawn-side consequence, and the only one: holding `5*N_R2_COARSE + 0.5*N_R2_TRIM_UNITS == 5*SCH_N_R2` at the new SCH_N_R2=51 moves N_R2_COARSE 48 -> 49 (255 um/leg). The fine ladder's unit count, DR-002's certified 0..-16 downward range and every other drawn parameter are untouched. Evidence: sim/output-voltage-tc/records/ (45-point PVT, schematic) and sim/output-voltage-tc-post-layout/records/ (extracted).
...
- **Array dummies are excluded, and the substrate correspondence is real drawn connectivity -- both new this increment.** The `pnp` and `res_high_po` counts above already exclude each array's dummy edge units (24 dropped this run); see "Retired since the last increment" above for both.

## Provenance

- Record ID: `20260923-070209-dbd57a9`
- `klt` version: `klt 0.2.0` (pinned, see `layout/requirements.txt`)
- KLayout engine version: `0.30.12`
- Repo state: `dbd57a9a24f72aadba1d69c567a49d8c5cb46f8b` on `feature/issue-285` (dirty)
