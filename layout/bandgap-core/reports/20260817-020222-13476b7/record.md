# Bandgap-core routed layout record: 20260817-020222-13476b7
...
| 2 | Resistor ladder at real unit count | MET | `res_r2` num=98 (= 2 legs x 49 coarse 5um units) + `res_trim` num=40 (= 2 legs x 20 fine 0.5um units) = 255 um/leg at DR-002 code 0, against design/bandgap_core.sch's `r_lseg*n_r2` = 255 um (delta +0 um); composed bbox 66,293 um^2 vs 80,000 um^2 budget |
| 3 | Extract: correct device classes + promoted pins | MET | device_counts={"nfet": 16, "pfet": 52, "pnp": 16, "res_high_po": 145}, pin_count=11 |
...
- [x] Composed bbox area (66,293 um^2) is within the < 0.05 mm^2 (80,000 um^2) budget, **at the real full-length ladder count** (98 coarse + 40 fine units)
...
| `VDD` | `core_mirror.TAP_N` + `core_mirror:VDD:far1` + `amp_pmirr.TAP_S` + `amp_input_pair.TAP_N` + `amp_cc.TAP_S` + `amp_pmirr:VDD:far0` + `amp_cc:VDD:far0` | yes | met1 + met2 x1 | VDD trunk: MPOUT/MPAMP and MP3/MP4 sources, MCC's drain+source (it is wired D=S=B=VDD, a MOS capacitor) -- every finger of all five, not one pad per block -- plus each PMOS group's n-well guard-ring tap (the reference's pfet bulk terminal) |
| `VSS` | `pnp_ctat:VSS trunk` + `amp_nload:VSS:far0` + `amp_nmirr:VSS:far0` + `amp_nload.TAP_S` + `amp_nmirr.TAP_S` + `pnp_ptat:VSS trunk` | yes | met1 + met2 x1 | VSS trunk: every finger of all four amp NMOS sources (MN1-MN4), both NMOS groups' substrate guard-ring taps, and both PNP base ties (the diode-connected PNPs' base sits on VSS) |
| `TAIL` | `core_mirror:TAIL:far0` + `amp_input_pair:TAIL:spine0` | yes | met1 | MPAMP drain to the amp input pair's common source |
| `GDRV` | `core_mirror:GDRV:far1` + `amp_cc:GDRV:spine0` + `amp_pmirr:GDRV:far1` + `amp_nmirr:GDRV:far1` | yes | met1 + met2 x3 | the amp's output -- MP4's and MN3's drains -- the core mirror's gate drive, and MCC's gate (the compensation cap sits from AOUT/GDRV to VDD), one node in the schematic and now one drawn node in the layout |
| `D1` | `amp_input_pair:D1:far1` + `amp_nload:D1:far1` + `amp_nmirr:D1:far0` | yes | met1 + met2 x2 | MP1's drain, MN1's diode-connected drain/gate, and MN3's gate |
| `D2` | `amp_input_pair:D2:far1` + `amp_nload:D2:far0` + `amp_nmirr:D2:far0` | yes | met1 + met2 x2 | MP2's drain, MN2's diode-connected drain/gate, and MN4's gate |
...
**11** of this cell's inter-block hops are drawn on met2 rather than met1, each entered and left through a via1 stack (met1 pad + `via.drawing` cut + met2 pad). met1 on this floorplan carries both every block's intra-block bus and every inter-block net, and the hops below had no met1 corridor at any lane, margin, block placement or search depth this repo can set -- layout/matching-plan.md Sections 7d-7o are the exhausted list. met2 is a genuinely independent conductor, and became one for sky130's curated deck only with 2AMLogic/klayout-tools#508 (merged via #511); before that its `metal2` role resolved to the same met1 layer this flow's own bussing already occupies. The escape is tried **strictly last**, after every met1 elbow, channel path and Z-detour has been drawn and rolled back, so met1 remains the primary plane -- see MET2_ESCAPE_NOTE.

| net | hop | via1 drops (um) | met2 path |
| --- | --- | --- | --- |
| `VOUT` | `core_mirror:VOUT:far0` -> `res_r2.R0_A` | (37.31, 50.27) -> (25.67, 1.85) | 3-point |
| `VDD` | `amp_pmirr:VDD:far0` -> `amp_cc:VDD:far0` | (172.86, 105.4) -> (224.06, 163.13) | 3-point |
| `VSS` | `pnp_ctat:VSS trunk` -> `amp_nload:VSS:far0` | (10.59, 0.31) -> (158.57, 50.67) | 3-point |
| `GDRV` | `core_mirror:GDRV:far1` -> `amp_cc:GDRV:spine0` | (37.31, 60.69) -> (49.0, 153.13) | 3-point |
| `GDRV` | `amp_cc:GDRV:spine0` -> `amp_pmirr:GDRV:far1` | (49.0, 153.13) -> (172.86, 112.22) | 3-point |
| `GDRV` | `amp_pmirr:GDRV:far1` -> `amp_nmirr:GDRV:far1` | (172.86, 112.22) -> (186.46, 112.053) | 3-point |
| `D1` | `amp_input_pair:D1:far1` -> `amp_nload:D1:far1` | (144.97, 60.823) -> (158.57, 59.49) | 3-point |
| `D1` | `amp_nload:D1:far1` -> `amp_nmirr:D1:far0` | (158.57, 59.49) -> (186.46, 105.9) | 3-point |
| `D2` | `amp_input_pair:D2:far1` -> `amp_nload:D2:far0` | (144.97, 64.157) -> (158.57, 52.67) | 4-point |
| `D2` | `amp_nload:D2:far0` -> `amp_nmirr:D2:far0` | (158.57, 52.67) -> (186.46, 104.567) | 3-point |
| `PN` | `amp_pmirr:PN:far0` -> `amp_nmirr:PN:far0` | (172.86, 106.9) -> (186.46, 107.233) | 4-point |
...
| met2 DRC (this repo's own) | clean | violation_count=0, via1 cuts=18, met2 polygons=7 |
| extract | ok | device_count=229, device_counts={"nfet": 16, "pfet": 52, "pnp": 16, "res_high_po": 145}, pin_count=11 |
...
- Record ID: `20260817-020222-13476b7`
- `klt` version: `klt 0.2.0` (pinned, see `layout/requirements.txt`)
- KLayout engine version: `0.30.10`
- Repo state: `13476b75a1e0d2167e12c987a9b93eadd58e46de` on `feature/issue-178` (dirty)
