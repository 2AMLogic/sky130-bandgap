# Bandgap-core routed layout record: 20260804-101943-25d02e6
...
| 4 | `klt lvs` clean | NOT MET | status=mismatch, mismatch_count=110 |
...
| LVS | mismatch | mismatch_count=110 |
...
Mismatch categories: `{"device.unmatched": 45, "net.unmatched": 63, "topology": 2}`.
...
- **Not LVS-clean.** `klt lvs` reports `mismatch` with `mismatch_count=110` against the xschem-derived reference netlist, and `devices.matched` is 0. The five causes above are the whole of it; none is hidden behind a number that moved.
...
- Record ID: `20260804-101943-25d02e6`
...
## Issue #72 bisect note (step 3 of 3)

This record is bisect **step 3** for issue #72 (`main`'s 0/0
`devices.matched`/`nets.matched` regression after #71): steps 1 and 2
**together**, i.e. both of the variables that changed between the closed #69
line and `main`, applied at once to `layout/bin/gen_bandgap_routed.py`:

- `RES_FLAVOR` = `"generic"` / `RES_CLASS` = `"res_generic_po"` (step 2), and
- `SUBSTRATE_SAME_NETS` = `[]` (step 1 -- the `hints.same_nets` declaration
  dropped).

Everything else -- the bus/route topology -- is unchanged from `main`, and
`layout/bandgap-core/reference.spice` is untouched.

Result: `lvs.combined.json` still reports `devices.matched: 0`,
`nets.matched: 0` (`mismatch_count: 110`).

### Bisect verdict (all three steps)
...
Neither variable, alone or together, recovers any correspondence. Per issue
#72's own reading of this outcome ("if (3) still shows 0/0, the difference
really is in the bus/route topology"), the cause is in the layout's own drawn
connectivity, not in the resistor flavour and not in the substrate hint --
and there is therefore no case at all for keeping `res_generic_po`, which
costs two extra `topology` mismatches (the reference names the schematic's
`res_high_po`) and buys nothing.

The cause, isolated from this baseline's own extracted netlist, is named in
the record of the fix run that follows this one in this directory.
