# Bandgap-core routed layout record: 20260804-090455-25d02e6
...
- Record ID: `20260804-090455-25d02e6`
...
## Issue #72 baseline note

This record reproduces `main`'s 0/0 `devices.matched`/`nets.matched`
regression (issue #72) with `layout/bin/gen_bandgap_routed.py` **unmodified**
(`RES_FLAVOR="high"`/`RES_CLASS="res_high_po"`,
`SUBSTRATE_SAME_NETS=[["vsubs", "VSS"]]`), on top of the same repo state
(`25d02e6`) the three bisect runs below it in this directory are diffed
against. `lvs.combined.json`: `devices.matched: 0`, `nets.matched: 0`,
`mismatch_count: 106` -- matching the finding in the issue body's data table
for `main` after #71.
