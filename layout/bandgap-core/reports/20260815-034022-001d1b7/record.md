# Bandgap-core routed layout record: 20260815-034022-001d1b7
...
| 2 | Resistor ladder at real unit count | MET | `res_r2` num=96 (= 2 legs x 48 coarse 5um units) + `res_trim` num=40 (= 2 legs x 20 fine 0.5um units) = 250 um/leg at DR-002 code 0, against design/bandgap_core.sch's `r_lseg*n_r2` = 250 um (delta +0 um); composed bbox 66,293 um^2 vs 80,000 um^2 budget |
| 3 | Extract: correct device classes + promoted pins | MET | device_counts={"nfet": 16, "pfet": 52, "pnp": 16, "res_high_po": 143}, pin_count=11 |
...
- [x] Composed bbox area (66,293 um^2) is within the < 0.05 mm^2 (80,000 um^2) budget, **at the real full-length ladder count** (96 coarse + 40 fine units)
...
**12** of this cell's inter-block hops are drawn on met2 rather than met1, each entered and left through a via1 stack (met1 pad + `via.drawing` cut + met2 pad). met1 on this floorplan carries both every block's intra-block bus and every inter-block net, and the hops below had no met1 corridor at any lane, margin, block placement or search depth this repo can set -- layout/matching-plan.md Sections 7d-7o are the exhausted list. met2 is a genuinely independent conductor, and became one for sky130's curated deck only with 2AMLogic/klayout-tools#508 (merged via #511); before that its `metal2` role resolved to the same met1 layer this flow's own bussing already occupies. The escape is tried **strictly last**, after every met1 elbow, channel path and Z-detour has been drawn and rolled back, so met1 remains the primary plane -- see MET2_ESCAPE_NOTE.

| net | hop | via1 drops (um) | met2 path |
| --- | --- | --- | --- |
| `TRIM_A` | `res_r2.R94_B` -> `res_trim.R0_A` | (74.26, 12.85) -> (117.25, 8.65) | 3-point |
| `VOUT` | `core_mirror:VOUT:far0` -> `res_r2.R0_A` | (37.31, 50.27) -> (25.67, 0.25) | 3-point |
| `VDD` | `amp_pmirr:VDD:far0` -> `amp_cc:VDD:far0` | (172.86, 105.4) -> (224.06, 163.13) | 3-point |
| `VSS` | `pnp_ctat:VSS trunk` -> `amp_nload:VSS:far0` | (10.59, 0.31) -> (158.57, 50.67) | 4-point |
...
| met2 DRC (this repo's own) | clean | violation_count=0, via1 cuts=20, met2 polygons=8 |
| extract | ok | device_count=227, device_counts={"nfet": 16, "pfet": 52, "pnp": 16, "res_high_po": 143}, pin_count=11 |
...
- Record ID: `20260815-034022-001d1b7`
- `klt` version: `klt 0.2.0` (pinned, see `layout/requirements.txt`)
- KLayout engine version: `0.30.10`
- Repo state: `001d1b76cab709562bb1e4b98b576b59f1bd9e81` on `feature/issue-170` (dirty)
