...
| 11 — Power delivery (structural) | unmet `no_evidence` | No `klt erc` supply-spec run or report exists in this repo yet. Tracked by this block's companion item-11 issue, #281 (open as of this manifest's commit) — the row exists here, `unmet`, exactly as issue #282's acceptance criteria requires, rather than being omitted while #281 is in flight. |
