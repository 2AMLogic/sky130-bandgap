...
def build_deck(pdk, process: str, supply_v: float, body: list[str]) -> str:
    head = [
        ...
        f"generated by sim/{SLUG}/run_res_array_resize.py, do not edit",
...
def analytic_resistances(n_r1: int, n_r2: int, trim_code: int = 0) -> dict[str, float]:
    ...
    head = 379.705147
    body = 324.827244
    r1_n = len(r1_segments_um(n_r1))
    r1_l = sum(r1_segments_um(n_r1))
...
def run_corner_set(run_dir, corners_dir, record_id, pdk, base_body, n_r1, n_r2, trim_code, timeout, tag):
    ...
    for process, supply in CORNERS:
        deck = build_deck(pdk, process, supply, body)
...
def verify(timeout: int, author: str, supersedes: str, allow_pdk_mismatch: bool, dry_run: bool) -> int:
    ...
    if dry_run:
        ...
        print(build_deck(pdk, *CORNERS[0], substitute_arrays(base_body, RESIZE_N_R1, RESIZE_N_R2, 0)))
        return 0
