...
| `sim/res-array-head-resistance/` | `run_res_array_head_resistance.py` | Compares two *topologies* of the same divider leg (issue #98, DR-003): one `res_high_po` device at the combined length vs. the routed layout's chain of N separately-contacted unit instances. A **structural** substitution in the netlisted body — one device line replaced by N series lines through new intermediate nodes — which no `deck.params` override can express |
| `sim/res-array-resize/` | `run_res_array_resize.py` | Re-derives and PVT-verifies `n_r1`/`n_r2` against that chained topology (issue #99). Extends the harness above with a candidate screen over integer `(n_r1, n_r2)` pairs, the full 15-point PVT matrix, a 1 °C hot-corner sweep, DR-002's trim ladder re-run on the resized baseline, and a single-device control — every point needing both the structural substitution *and* a per-point `.param` edit, i.e. both reasons the two rows above exist at once |
