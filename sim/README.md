...
| `sim/res-array-head-resistance/` | `run_res_array_head_resistance.py` | Structural (not parametric) substitution: replaces each divider leg's single netlisted device line with an N-instance series chain, to measure the routed array's real per-instance head resistance (issue #98 / DR-003). Pinned to the pre-#99 schematic on purpose — see its module docstring |
| `sim/res-array-resize/` | `run_res_array_resize.py` | Re-derives `n_r1`/`n_r2` against the chained-array values and re-verifies over the full 15-point PVT matrix plus a trim-range recheck (issue #99). Same `.param`-in-the-netlisted-body reason as `run_trim_sweep.py`, over three parameters and three phases; also carries a raw-deck phase that validates the schematic's series-leg model against an explicit N-instance chain |
